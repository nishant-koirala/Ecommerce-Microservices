 ...
 * use it — appending Unsplash sizing params for external/CDN URLs, but serving
 * locally uploaded images (/images/...) as-is; otherwise fall back to a
  ...
  product(product: { id: number; sku: string; imageUrl?: string | null }, size = 600): string {
    if (product.imageUrl) {
      return product.imageUrl.startsWith('http')
        ? `${product.imageUrl}?auto=format&fit=crop&w=${size}&q=80`
        : product.imageUrl;
    }
